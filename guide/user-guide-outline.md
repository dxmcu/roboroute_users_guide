...
若后期的业务订单请求中包含某 “业务类型” ，而您又希望该机器人有机会被分配此业务，您可以在编辑场景时为此机器人添加一个或多个 “可执行的业务类型”。

自主充电类型默认为 ”CHARGE“，您可以修改为其他任意字符串。但请注意，在本机器人适配的充电站中，其工作站类型的操作里，一定要包含和本机器人 ”自主充电类型“ 完全相同的字符串。
...
上文提到，通过 Roboshop 直接导出的 xml 场景文件，不包含具体的工作站信息。默认情况下，首次导出的 xml 场景文件包含三个无任何操作描述的工作站类型，分别是：Charge Location，Transfer Location，和 Working Location。您可以任意添加和删除工作站类型，并且在工作站类型中添加允许机器人执行的操作（请见 11.4 节）。
...
4. 在 Viewer 中将运行模式切换至操作模式，使用一键派遣所有机器人功能，将机器人在线状态全部置为在线可用；
...
| Wait                    | orientation  | 右手坐标系为基准的角度(单位：度，-180~180) | optional     |
| WaitKey                 | DI的序号     | 0 / 1。当DI从0变为1时，等待结束            | **required** |
