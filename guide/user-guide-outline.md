...
本章节默认 Kernel 已加载场景文件并处于操作模式，机器人已经释放控制权给 RoboRoute。
...
![img](../pictures/01/clip_image093.png)
 ...
 #### 8.5.3 撤销机器人正在执行的业务订单

在需要撤回订单的机器人图标上单击鼠标右键，弹出如下菜单栏，选择撤销业务订单，根据实际需求选择撤销订单的方式。

![img](D:/Workspace/git_project/roboroute_users_guide/pictures/01/clip_image078.png)
...
| 订单撤回方式     | 释义                                                         |
| ---------------- | ------------------------------------------------------------ |
| 并到达下一节点   | 机器人继续执行已经生成的调度指令，执行完毕后 RoboRoute 将撤销当前业务订单。 |
| 并立即停止机器人 | 机器人将在到达下一个节点后立即停止，同时 RoboRoute 撤销当前执行的订单。 |
